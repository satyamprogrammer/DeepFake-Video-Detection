...
def affine_wrap_face_swap(src_img, dst_img, output_str):

    #randon resize
    ratio = random.uniform(1,0.7)
    src_img = cv2.resize(src_img, None, fx=ratio, fy=ratio)

    #gaussian blur
    src_img = cv2.GaussianBlur(src_img,(5,5),cv2.BORDER_DEFAULT)
    ...
    dir_path = os.path.dirname(output_str)
    if not os.path.isdir(dir_path):
        os.makedirs(dir_path)

    cv2.imwrite(output_str, output)
